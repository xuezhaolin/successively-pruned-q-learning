...
class DQN(RlAlgorithm):
    ...
    def initialize_replay_buffer(self, examples, batch_spec, async_=False):
        ...
        if self.prioritized_replay:
            ...
            ReplayCls = (AsyncUniformReplayFrameBuffer if async_ else UniformReplayFrameBuffer)
    ...
    def loss(self, samples):
        ...
        with torch.no_grad():
            ...
            if self.double_dqn and not self.clipped_dqn:
                next_qs = self.agent(*samples.target_inputs)
                next_a = torch.argmax(next_qs, dim=-1)
                target_q = select_at_indexes(next_a, target_qs)
            else:
                if not self.double_dqn and self.clipped_dqn:
                    next_qs = self.agent(*samples.target_inputs)
                    next_a = torch.argmax(next_qs, dim=-1)
                    min_a=torch.argmin(next_qs,dim=-1)
                    min_q=select_at_indexes(min_a,next_qs)
                    next_q=select_at_indexes(next_a,next_qs)
                    target_qq = select_at_indexes(next_a, target_qs)
                    #minimal target_q and next_q
                    if self.min_va:
                        target_q=torch.as_tensor(np.minimum(next_q.numpy(),target_qq.numpy()))
                    else:
                        target_q=torch.as_tensor(np.maximum(np.minimum(next_q.numpy(),target_qq.numpy()),min_q.numpy()))
                elif not self.double_dqn and self.sc_dqn:
                    next_qs = self.agent(*samples.target_inputs)
                    beta=3
                    beta_qs=target_qs-beta*(target_qs-next_qs)
                    next_a=torch.argmax(beta_qs,dim=-1)
                    target_q=select_at_indexes(next_a,target_qs)
