...
class AsyncRlBase(BaseRunner):
    ...
    def train(self):
        ...
        while True:  # Run until sampler hits n_steps and sets ctrl.quit=True.
            ...
            with logger.prefix(f"opt_itr #{itr} "):
                ...
                traj_infos = (list() if self._eval else drain_queue(self.traj_infos_queue))
    ...
    def log_diagnostics(self, itr, sampler_itr, throttle_time, prefix='Diagnostics/'):
        ...
        replay_ratio = (new_updates * self.algo.batch_size * self.world_size /max(1, new_samples))
        ...
        #     logger.record_tabular('Iteration', itr)
            logger.record_tabular('SamplerIteration', str(sampler_itr)+'/'+str(self.n_itr))
            logger.record_tabular('Step', str(sampler_itr*self.sampler_batch_size)+'/'+str(self.n_itr*self.sampler_batch_size))
            logger.record_tabular('Ratio',str(round(sampler_itr/self.n_itr,4)*100)+'%')
...
class AsyncRlEval(AsyncRlBase):
    """
    Asynchronous RL with offline agent performOance evaluation.
    """
